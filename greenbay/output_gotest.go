...
func (r *GoTest) Populate(jobs <-chan amboy.Job) error {
	...
	if err != nil {
		return errors.Wrap(err, "generating gotest results")
	}
...
func (r *GoTest) ToFile(fn string) error {
	if err := ioutil.WriteFile(fn, r.buf.Bytes(), 0644); err != nil {
		return errors.Wrapf(err, "writing output to %s", fn)
	}
...
func printTestResult(w io.Writer, check CheckOutput) bool {
	_, _ = fmt.Fprintln(w, "=== RUN", check.Name)
	if check.Message != "" {
		_, _ = fmt.Fprintln(w, "    message:", check.Message)
	}

	if check.Error != "" {
		_, _ = fmt.Fprintln(w, "    error:", check.Error)
	}
