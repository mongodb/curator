...
func TestSimpleAuthenticator(t *testing.T) {
	...
	// constructor avoids nils
	usr := NewBasicUser("id", "name", "email", "pass", "key", []string{}, false)
	...
	usr2 := NewBasicUser("id2", "name", "email", "pass", "key", []string{}, false)
	assert.False(auth.CheckAuthenticated(usr2))

	usr3 := NewBasicUser("id3", "name", "email", "pass", "key", []string{"admin"}, false)
	usr3broken := NewBasicUser("id3", "name", "email", "pass", "yek", []string{"admin"}, false)
